...
const NODE_COLORS = ["#22D3EE", "#2563EB", "#38BDF8", "#7C5CFF", "#00FFA8", "#A78BFA", "#60A5FA", "#94A3B8"];
...
export function KnowledgeGraphPage() {
  ...
  return (
    ...
    >
      <div className="cockpit-surface moto-cut-card p-4">
        <div className="ai-light-sheen opacity-35" />
        <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          <div className="relative">
            <p className="text-xs font-bold uppercase tracking-[0.24em] text-neon-cyan">GraphRAG</p>
            <h1 className="moto-heading mt-1 text-3xl">Knowledge Graph</h1>
            <p className="mt-1 text-sm text-muted-foreground">Premium dark graph visualization for live MotoAI schema data.</p>
          </div>

          <div className="relative flex flex-col gap-2 lg:flex-row lg:items-center">
            <div className="relative min-w-[260px] lg:w-[340px]">
            ...
            <div className="flex flex-wrap gap-2">
              <Button aria-label="Refresh graph" size="icon" variant="secondary" onClick={() => query.refetch()} disabled={query.isFetching}>
                <RefreshCw className={query.isFetching ? "h-4 w-4 animate-spin" : "h-4 w-4"} />
              </Button>
              <Button aria-label="Zoom in" size="icon" variant="outline" onClick={() => graphRef.current?.zoomIn()}>
                <ZoomIn className="h-4 w-4" />
              </Button>
              <Button aria-label="Zoom out" size="icon" variant="outline" onClick={() => graphRef.current?.zoomOut()}>
                <ZoomOut className="h-4 w-4" />
              </Button>
              <Button aria-label="Fit view" size="icon" variant="outline" onClick={() => graphRef.current?.fitView()}>
                <Expand className="h-4 w-4" />
              </Button>
              <Button aria-label="Fullscreen graph" size="icon" variant="outline" onClick={() => graphRef.current?.fullscreen()}>
